...
$d\psi/dx = -A for 0 < x < a$ $d\psi/dx = +A for -a < x < 0$ $<T> = (\hbar^2/2m)\int_{-a}^{a}|d\psi/dx|^2 dx$ $= (\hbar^2/2m) \times 2\int_{0}^{a} A^2 dx$ $= (\hbar^2/2m) \times 2A^2 a$ $= (\hbar^2/2m) \times$ $<E> = (\hbar^2/2m) \times 
...
\textbf{ANSWER (Case ii): Optimal value b = 0 (triangular function gives minimum)}
...
$= 2\int_{0}^{a} (x^4 - 2a^2 x^2 + a^4) dx$ $= 2[x^5/5 - 2a^2 x^3/3 + a^4 x]_{0}^{a}$ $= 2(a^5/5 - 2a^5/3 + a^5)$ $= 2a^5(1/5 - 2/3 + 1)$ $= 2a^5(3/15 - 10/15 + 15/15)$ $= 2a^5(8/15)$ $Derivative: d\psi/dx = 2Ax$
...
\textbf{(All terms are even functions, so we integrate over symmetric limits)}
...
\textbf{(Even function integrated over symmetric limits)}
...
$Overlap: \int_{-a}^{a} \psi_{0}\psi_{t} dx =$ $Mean-square deviation: \Delta^2 =$ $Interpretation: Smaller \Delta^2 indicates better approximation to exact ground state.$

\textbf{PART (e): NODES OF OPTIMAL QUARTIC AND INTERPRETATION}
...
\subsection*{At the stationary point (d<E>/dr = 0):}

$\bullet The energy is minimized with respect to the parameter r$ $\bullet This provides an upper bound: <E> \geq E_{0} (variational theorem)$ $\bullet The optimal function best approximates the true ground state within this family$
...
\textbf{Linear Potential V(x) = g|x| - Rayleigh-Ritz Method}

\subsection*{STEP 1: Basis Functions}

$f\:2081(x) = exp(-x^{2}/a^{2}) [EVEN function]$ $f\:2082(x) = x\[CenterDot]exp(-x^{2}/a^{2}) [ODD function]$

\textbf{STEP 2: Overlap Matrix Elements S\:1d62\:2c7c = \:27e8f\:1d62|f\:2c7c\:27e9}
...
\textbf{T\:2081\:2081 = -\.bd\int f\:2081\[CenterDot]f\:2081'' dx =}
...
\textbf{T\:2082\:2082 = -\.bd\int f\:2082\[CenterDot]f\:2082'' dx =}
...
\textbf{All off-diagonal elements are ZERO due to parity symmetry.}
...
\textbf{Eigenvectors (coefficients [c\:2081, c\:2082]):}
...
\textbf{Since H and S are block-diagonal, eigenvalues are:}

\subsection*{E\:2081 = H\:2081\:2081/S\:2081\:2081 =}

\subsection*{E\:2082 = H\:2082\:2082/S\:2082\:2082 =}

STEP 8: Comparison with Exact (Analytic) Results For the linear potential V(x) = g|x|, the exact solution

\textbf{involves Airy functions. The energy eigenvalues are:}

$E\:2099 = g^(2/3) \[CenterDot] |\alpha\:2099|$ $where \alpha\:2099 are the zeros of the Airy function Ai(-z).$
...
\textbf{E\:2081 0.898942 2.33811 \!\(\*RowBox[{\"\\\"61.55\\\"\"}]\)}

\textbf{E\:2082 2.29788 4.08795 \!\(\*RowBox[{\"\\\"43.79\\\"\"}]\)}

\textbf{------------------------------------------------------------------------}

\subsection*{STEP 9: Visualization of Results}

\subsection*{STEP 10: Analysis and Comments}
...
$\bullet Linear potential g|x| is a 'V-shaped' well$ $\bullet With g = \hbar^{2}/(ma) = 1, length scale set by a = 1$ $\bullet Ground state: concentrated near x = 0, no nodes$ $\bullet Excited state: has node at x = 0 (odd parity)$

\subsection*{6. WHY THE METHOD WORKS WELL:}

$\bullet Parity structure exactly preserved$ $\bullet Variational freedom via linear combinations$

\section*{Figures}

% The notebook contains 3 figures.
% To include them, export the graphics from Mathematica using:
%   Export["figure_N.png", graphicsObject]
% Then place the PNG files in the HW 8-1 pb 5-all_figures/ directory.

\begin{figure}[H]
\centering
% TODO: Export figure from Mathematica
