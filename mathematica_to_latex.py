...
class MathematicaToLatexConverter:
    ...
    GREEK_LETTERS = {
        ...
        r'\[Nu]': r'$\nu$',
        r'\[Xi]': r'$\xi$',
        r'\[Omicron]': r'$o$',  # Standard LaTeX doesn't have \omicron command; use 'o' directly
        r'\[Pi]': r'$\pi$',
        r'\[Rho]': r'$\rho$',
        r'\[Sigma]': r'$\sigma$',
        r'\[Tau]': r'$\tau$',
        ...
        while i < len(grid_inner):
            if grid_inner[i] == '{':
                ...
                if brace_count == 0:
                    ...
                    # Use a more robust pattern that handles escaped characters within cells
                    # (?:\\\\.|[^"\\\\])* matches either an escaped character or any non-quote/backslash character
    ...
    def extract_cell_content(self, cell: Dict) -> Optional[str]:
        ...
        if cell_type == 'Input':
            ...
            if string_content:
                ...
                # Remove actual newline characters from the result
